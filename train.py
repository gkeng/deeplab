...
flags.DEFINE_enum(
    "optimizer", "momentum", ["momentum", "adam"], "Which optimizer to use."
)
...
flags.DEFINE_float(
    "decay_steps", 0.0, "Decay steps for polynomial learning rate schedule."
)

flags.DEFINE_float(
    "end_learning_rate", 0.0, "End learning rate for polynomial learning rate schedule."
)
...
flags.DEFINE_float("adam_learning_rate", 0.001, "Learning rate for the adam optimizer.")
...
flags.DEFINE_list(
    "train_crop_size", [480, 640], "Image crop size [height, width] during training."
)
...
flags.DEFINE_float(
    "max_scale_factor", 2.0, "Maximum scale factor for data augmentation."
)
...
def main(unused_argv):
    ...
    assert (
        FLAGS.train_batch_size % config.num_clones == 0
